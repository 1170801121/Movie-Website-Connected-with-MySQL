...
DROP TABLE IF EXISTS staring;
DROP TABLE IF EXISTS invest;
DROP TABLE IF EXISTS `movie`;
DROP TABLE IF EXISTS staff;
DROP TABLE IF EXISTS `user`;
DROP TABLE IF EXISTS company;
DROP VIEW IF EXISTS ACTOR_EXPRIENCE;
DROP VIEW IF EXISTS DIRECTOR_EXPRIENCE;
DROP VIEW IF EXISTS COMPANY_EXPRIENCE;
DROP VIEW IF EXISTS MOVIE_SCORE;
SET FOREIGN_KEY_CHECKS=0;
-- Table structure for 'staff'

CREATE TABLE staff (
    Sname varchar(30) COMMENT '中文名',
    foreignName varchar(30) DEFAULT NULL,
    constellation varchar(20) DEFAULT NULL,
    Stype BIT(2) COMMENT '角色(0表示演员, 1表示导演, 3表示既是导演，也是演员)', 
    birthdate date DEFAULT NULL COMMENT '出生日期',
    birthplace varchar(30) DEFAULT NULL COMMENT '出生地点',
    family varchar(100) DEFAULT NULL COMMENT '家庭成员',
    PRIMARY KEY (Sname)
);
INSERT INTO staff VALUES
    ('鲁本·弗雷斯彻', 'Ruben Fleischer', '天蝎座', 1, '1974-10-31', '美国, 华盛顿, 哥伦比亚特区', ''),
    ('汤姆·哈迪', 'Tom Hardy', '处女座', 0, '1977-09-15', '英国, 伦敦, 哈默史密斯', '');
...
-- Table structure for 'staring'
CREATE TABLE staring(
    staffName varchar(30) NOT NULL COMMENT '中文名',
    movie_id int(10) NOT NULL AUTO_INCREMENT COMMENT '电影ID',
    FOREIGN KEY (staffName) REFERENCES staff(Sname),
    FOREIGN KEY (movie_id) REFERENCES movie(movie_id)
);
INSERT INTO staring VALUES
    ('鲁本·弗雷斯彻', 1),
    ('汤姆·哈迪', 1);
...
CREATE TABLE company (
    Cname varchar(30),
    CEO varchar(20),
    Ctype BIT(2) COMMENT '0表示制作, 1表示发行, 2代表都是)',
    PRIMARY KEY (Cname)
);
...
CREATE TABLE invest (
    ...
    movie_id int(10),
    Itype BIT(1),
    cost decimal(8, 1) DEFAULT NULL COMMENT '投资成本',
    `boxOffice` decimal(8,1) DEFAULT NULL COMMENT '累计票房',
    FOREIGN KEY (Cname) REFERENCES company(Cname),
    FOREIGN KEY (movie_id) REFERENCES movie(movie_id)
);
INSERT INTO invest VALUES
    ('CompA', 1, 1, 432432, 324343),
    ('CompB', 1, 0, 324324, 432432);
...
-- INDEX
CREATE INDEX idx_Mo_na ON movie(name);
CREATE INDEX idx_Mo_ty ON movie(type);
CREATE INDEX idx_Mo_sc ON movie(score);
CREATE INDEX idx_Mo_da ON movie(releaseDate);
CREATE INDEX idx_Mo_po ON movie(releasePoint);
CREATE INDEX idx_Co_id ON comments(movie_id);


-- VIEW
CREATE VIEW COMPANY_EXPRIENCE AS
SELECT Cname, AVG(invest.cost), AVG(invest.boxOffice) FROM movie JOIN invest ON invest.movie_id = movie.movie_id
GROUP BY Cname;